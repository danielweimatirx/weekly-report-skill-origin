...
**读者**：团队 leader 的上级。时间有限，想看：业务进展、风险/阻塞/需决策事项、跨团队协作。**不关心** PR/Issue 计数、代码细节、流水账。

**组织方向**：按业务主题 / 项目聚合（如"金盘 ChatBI"、"MOI 原型"、"官网改版"）。成员作为每个项目下的参与者标签，不单独列成员表。
...
4. **理解 PR/Issue 关联**：互相引用、共同 label、同 repo → 合并讲一件事，不要拆散成多条
...
7. **#号是凭证不是章节**：`#3421` 在叙述中作为超链接引用出现，不作为结构层级
