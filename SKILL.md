...
读取 CLAUDE.md，查找"周报配置"段落。逐项检查以下 4 个必填字段：

1. **GitHub Token** — CLAUDE.md 中有 `GitHub Token: xxx` 这一行
2. **GitHub 用户名** — CLAUDE.md 中有 `GitHub 用户名: xxx` 这一行
3. **岗位** — CLAUDE.md 中有 `岗位: xxx` 这一行
4. **仓库范围** — CLAUDE.md 中有 `仓库范围: xxx` 这一行

**4 个字段全部存在才能生成周报。缺任何一个都必须先问用户，不能跳过，不能用默认值代替。**

跳转规则：
- 4 个全有 → 跳到第 2 节（推断日期）
- 缺哪个 → 跳到对应步骤补全（1.1 token / 1.2 用户名 / 1.3 岗位 / 1.4 仓库范围）
- 没有 token → 从步骤 1.1 开始
