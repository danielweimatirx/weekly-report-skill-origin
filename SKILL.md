...
| 代码叙述（差）| 业务叙述（好）|
|---|---|
| "重构 React 状态管理，引入 Zustand" | "前端重构解决了金盘反馈的 UI 状态丢失问题，同时降低后续 feature 开发成本" |
| "升级 Node 18 → 22" | "引擎升级打通新版 debugger 能力，为下季度 MOI 云调试做准备" |
| "修了 18 个 bug" | "集中收掉 ChatBI 权限类缺陷（18 个），客户工单从 23 清到 5" |

**每个技术动作问自己 "so what"——业务上有什么意义？** 没答案的动作不配进周报。
...
1. **不堆 PR/Issue 数字当内容**：数字只在**对比/异常**时有意义
2. **不编造业务价值**：LLM 不要瞎编 "提升 XX% 效率"、"降低了 YY 风险" 这种没来源的话。只讲**数据里明确看到**的东西
3. **深入读 body 和 comments**：PR/Issue 的描述、讨论评论里才有上下文（需求背景、决策过程、阻塞原因）。不要只看 title 和 state
4. **理解逻辑关系**：PR/Issue 之间有引用（#123、relates to）、共同 label、同 repo → 是一件事的多个方面。合并讲，不要拆散
5. **Memoria 补充**：第 4 节的记忆搜索不能跳过，团队/项目角度也要搜
6. **#号是超链接不是章节**：形如 "#3421" 在叙述中作为凭证出现，不是结构层级
7. **分层呈现**（当 `subtree_groups` 多于 1 个时）：对外按子团队分段讲；对内先按成员讲，然后可以在段落里标注"该成员属于子团队 X"
