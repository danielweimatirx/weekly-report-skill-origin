...
同一件事有两种写法，不分岗位，都要用价值叙述：

| 过程叙述（差）| 价值叙述（好）|
|---|---|
| "重构 React 状态管理，引入 Zustand" | "前端重构解决金盘反馈的 UI 状态丢失问题，同时降低后续 feature 开发成本" |
| "修了 18 个 bug" | "集中收掉 ChatBI 权限类缺陷（18 个），客户工单从 23 清到 5" |
| "合并了 12 个官网 PR" | "官网改版主体完成：首页 / About Us / Research / 法律页全面上线，市场侧可启用新素材" |
| "开了 8 次需求评审会" | "完成 ChatBI 一期需求锁定，研发侧可启动，目标 Q3 上线" |
| "提了 29 个 Issue" | "本周跟进金盘客户反馈 29 条，关闭 16 条，p0 权限类新增 5 条需下周拉齐方案" |
| "升级 Node 18 → 22" | "引擎升级打通新版 debugger 能力，为下季度 MOI 云调试做准备" |
| "写了 3 篇博客" | "博客自动发布链路打通 + 发布 3 篇，其中 Memoria 介绍单篇 3k 阅读" |
| "拜访了 3 个客户" | "锁定金盘二期商机（预估 ¥200w），进入商务谈判阶段" |

**每个动作都问 "so what"——对业务 / 客户 / 团队效率产生了什么？** 没答案的动作不配进周报，或只在专项投入章节顺带提一笔。
